...
void activity_selection(int *,int *,int,struct node *,int *);
...
int main()
{
	...
	for(i=0;i<n;i++)
		printf("%d ",finish[i]);
	int index=0;
	activity_selection(start,finish,n,ans,&index);
...
void activity_selection(int *start,int *finish,int n,struct node *ans,int *index)
{
